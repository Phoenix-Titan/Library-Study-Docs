...
4. [Functions: Overloading, Defaults, Lambdas, std::function, Templates Intro](#4-functions-overloading-defaults-lambdas-stdfunction-templates-intro) **[B/I]**
5. [Classes & RAII: The Rule of 0/3/5, Copy vs Move](#5-classes--raii-the-rule-of-035-copy-vs-move) **[I]**
6. [Resource Management & Smart Pointers](#6-resource-management--smart-pointers) **[I/A]**
7. [Move Semantics & Value Categories](#7-move-semantics--value-categories) **[I/A]**
8. [STL Containers: vector, string, map, set & Friends](#8-stl-containers-vector-string-map-set--friends) **[I]**
9. [Iterators, Algorithms & Ranges (C++20)](#9-iterators-algorithms--ranges-c20) **[I/A]**
10. [Templates, Concepts & Generic Programming](#10-templates-concepts--generic-programming) **[A]**
11. [Error Handling: Exceptions, std::expected, std::optional](#11-error-handling-exceptions-stdexpected-stdoptional) **[I/A]**
12. [Inheritance, Polymorphism & std::variant](#12-inheritance-polymorphism--stdvariant) **[I/A]**
13. [File System, OS & Processes](#13-file-system-os--processes) **[I]**
14. [Concurrency: threads, mutex, atomic, async, coroutines](#14-concurrency-threads-mutex-atomic-async-coroutines) **[A]**
15. [Modern Build & Dependency Management (CMake, vcpkg, Conan, Modules)](#15-modern-build--dependency-management-cmake-vcpkg-conan-modules) **[A]**
16. [Production-Grade C++: Sanitizers, clang-tidy, Core Guidelines](#16-production-grade-c-sanitizers-clang-tidy-core-guidelines) **[A]**
17. [Testing & Maintainability (GoogleTest/Catch2, CI, Layout)](#17-testing--maintainability-googletestcatch2-ci-layout) **[A]**
18. [Performance: Cache, Allocations, constexpr, Profiling](#18-performance-cache-allocations-constexpr-profiling) **[A]**
...
- **Clang** (`clang++`) — the LLVM compiler. Excellent diagnostics, the sanitizers (§16), and `clang-format`/`clang-tidy`. Default on macOS (shipped as part of Xcode). Clang 18/19 in 2026.
...
This is why C++ traditionally splits code into **headers** (`.hpp` — declarations: "this function exists, here's its signature") and **source files** (`.cpp` — definitions: the actual body). Every `.cpp` that wants to call `foo` `#include`s the header to learn its signature; exactly one `.cpp` provides the body. **Modules** (§15) are the C++20 replacement for this header machinery, but headers still dominate in 2026.
...
Compiling one file by hand is fine; real projects have hundreds. **CMake** is the de-facto standard *build-system generator*: you describe your project in a `CMakeLists.txt`, and CMake generates the actual build files for your platform (Makefiles on Linux, a Visual Studio solution on Windows, Ninja files, etc.). You'll meet CMake in depth in §15; here's the minimum to build a project today.
...
int main() {
    ...
    // Integer division truncates toward zero; mixing signed/unsigned is dangerous (§19).
...
auto&& fwd = make_thing();          // forwarding reference (§7)
...
- **`constinit`** (C++20) — "this static/global must be initialized at compile time" (avoids the *static initialization order fiasco*, §19).
...
## 4. Functions: Overloading, Defaults, Lambdas, std::function, Templates Intro

### 4.1 Declaring and defining functions **[B]**
...
### 4.2 Overloading and default arguments **[B]**
...
A caution: only *trailing* parameters can have defaults, and defaults live in the declaration (header), not the definition. Overload resolution + implicit conversions can surprise you (§19) — when in doubt, make conversions explicit.

### 4.3 Lambdas and closures — the workhorses of modern C++ **[B/I]**

A **lambda** is an anonymous function you can write inline, right where you use it. Lambdas are everywhere in modern C++ because algorithms and ranges (§9) take callables. A lambda can **capture** variables from its surrounding scope, becoming a **closure** (a function bundled with some captured state). Understanding capture is essential and a common source of bugs.
...
- `[&x]` — capture `x` **by reference** (the closure refers to the outer `x`; **danger**: if the outer `x` dies before the lambda runs, you have a dangling reference — see §19).
...
### 4.4 `std::function` and passing callables around **[I]**

Each lambda has its own unique, unnameable type, which is great for performance (the compiler can inline it) but awkward when you want to *store* a callable in a variable or member, or accept "any callable with this signature" as a parameter. `std::function<R(Args...)>` is a **type-erased** wrapper that can hold any callable matching that signature — a lambda, a function pointer, a functor. It costs a small runtime indirection (and possibly a heap allocation), so prefer a template parameter (§4.5) in hot paths, but `std::function` is invaluable for callbacks, event handlers, and storing heterogeneous callables.
...
In §10 you'll learn to *constrain* templates with **concepts** so that misuse produces a clear error ("T must be comparable") instead of a wall of template gibberish.

---

## 5. Classes & RAII: The Rule of 0/3/5, Copy vs Move
...
### 5.1 A class, member by member **[I]**
...
### 5.2 The destructor and RAII in action **[I]**
...
### 5.3 The Rule of 0, 3, and 5 — the most important class-design rule **[I]**
...
### 5.4 `=default`, `=delete`, and controlling copyability **[I]**
...
### 5.5 Operator overloading & three-way comparison `<=>` **[I]**
...
## 6. Resource Management & Smart Pointers
...
### 6.1 Why heap allocation exists, and why to avoid it **[I]**
...
### 6.2 `std::unique_ptr` — exclusive ownership (your default) **[I]**
...
### 6.3 `std::shared_ptr` — shared ownership (use sparingly) **[I/A]**
...
### 6.4 `std::weak_ptr` — breaking reference cycles **[A]**
...
### 6.5 Ownership models — a decision table **[I]**
...
| Optional non-owning view of a contiguous range | `std::span<T>` | see §8 |
...
## 7. Move Semantics & Value Categories
...
### 7.2 Value categories: lvalue, rvalue, xvalue **[I/A]**
...
### 7.3 Move constructors and `noexcept` **[I/A]**

Recall the Rule of 5 (§5.3): the move constructor `T(T&& other) noexcept` and move assignment steal `other`'s resources. Marking them `noexcept` is important: containers like `std::vector`, when they grow and must relocate their elements, will only *move* them (instead of copying) if the move is `noexcept` — otherwise they copy, to preserve the strong exception guarantee. **Always mark your moves `noexcept`.**

### 7.4 Perfect forwarding and forwarding references **[A]**
...
### 7.5 Copy elision / RVO — moves you don't even pay for **[I/A]**
...
## 8. STL Containers: vector, string, map, set & Friends

The **Standard Template Library (STL)** gives you battle-tested, RAII-managed, generic containers and algorithms. Using them instead of hand-rolled arrays and pointers is most of what makes C++ safe and productive. They own their memory, clean up automatically, grow as needed, and compose with algorithms and ranges (§9).

### 8.1 `std::vector` — the default container **[I]**
...
A key gotcha: operations that grow a vector (`push_back`, `insert`) may **reallocate**, which **invalidates all iterators, pointers, and references** into it (§19). `reserve()` mitigates this and also avoids wasted reallocations.

### 8.2 `std::array` and `std::string` **[I]**
...
### 8.3 `std::string_view` and `std::span` — non-owning views **[I]**

These two C++17/20 types are *views*: they refer to data they don't own, storing just a pointer and a length. They make passing read-only data cheap and avoid copies, but you must ensure the underlying data outlives the view (a dangling view is UB — §19).
...
### 8.4 Associative containers: maps and sets **[I]**
...
### 8.5 Other containers & a complexity cheat-sheet **[I]**
...
The practical heuristic: **use `vector` unless you have a measured reason not to.** Contiguous memory beats theoretically-better Big-O on modern hardware surprisingly often (§18).

---

## 9. Iterators, Algorithms & Ranges (C++20)
...
### 9.1 Iterators and the half-open range `[begin, end)` **[I]**
...
### 9.2 The classic `<algorithm>` library **[I]**
...
### 9.3 Ranges and views (C++20) — the modern way **[I/A]**
...
### 9.4 `std::format` and `std::print` — type-safe text output **[I]**
...
## 10. Templates, Concepts & Generic Programming
...
### 10.1 Class templates **[A]**

Just as functions can be templates (§4.5), so can classes. `std::vector<T>`, `std::unique_ptr<T>`, and `std::map<K,V>` are all class templates. You parameterize a class over types (or even values).
...
### 10.2 Concepts (C++20) — constraints that make templates usable **[A]**
...
### 10.4 Type traits and `if constexpr` **[A]**
...
## 11. Error Handling: Exceptions, std::expected, std::optional
...
### 11.1 Exceptions and exception safety **[I]**
...
Mark functions that can't throw `noexcept` (it enables optimizations and is required for moves used by containers, §7.3). Never let an exception escape a destructor — if a destructor throws during unwinding, the program calls `std::terminate`.

### 11.2 `std::optional` — "a value, or nothing" **[I]**
...
### 11.3 `std::expected` — "a value, or an error" (C++23) **[I/A]**
...
### 12.2 Object slicing — a classic trap **[I/A]**
...
### 12.3 Prefer composition over inheritance **[I/A]**
...
### 12.4 `std::variant` and `std::visit` — closed sets without inheritance **[I/A]**
...
### 13.1 Paths and the filesystem library **[I]**
...
### 13.2 Reading and writing files with streams **[I]**
...
### 13.3 Environment variables, arguments, and system info **[I]**
...
### 13.4 Running external commands / processes **[I]**
...
## 14. Concurrency: threads, mutex, atomic, async, coroutines
...
### 14.1 Threads and `std::jthread` **[A]**
...
### 14.3 `std::atomic` — lock-free synchronization **[A]**
...
### 14.4 Futures, promises, and `std::async` **[A]**
...
### 14.6 A word on coroutines (C++20) **[A]**
...
## 15. Modern Build & Dependency Management (CMake, vcpkg, Conan, Modules)
...
# Enable testing and pull in a test executable (see §17):
enable_testing()
...
### 15.2 Dependency management: vcpkg and Conan **[A]**
...
### 15.3 Modules (C++20) and their state **[A]**
...
### 16.1 The sanitizers — runtime bug detectors **[A]**
...
### 16.2 Static analysis: clang-tidy and warnings **[A]**
...
### 16.3 The C++ Core Guidelines and GSL **[A]**
...
### 16.4 Secure coding: avoiding undefined behavior **[A]**
...
## 17. Testing & Maintainability (GoogleTest/Catch2, CI, Layout)
...
### 17.1 Unit testing with GoogleTest and Catch2 **[A]**
...
### 17.2 Project layout & continuous integration **[A]**
...
├── benchmarks/             # performance tests (§18)
├── .clang-tidy
├── .clang-format
└── docs/                   # documentation
```
...
### 17.3 API/ABI stability and documentation **[A]**
...
## 18. Performance: Cache, Allocations, constexpr, Profiling
...
### 18.1 Cache awareness — the dominant modern factor **[A]**
...
### 18.2 Avoid needless allocations and copies **[A]**
...
// 3. Take string_view / span params to avoid copying when you only read (§8.3).

// 4. Move instead of copy for large objects you're done with (§7):
result.push_back(std::move(big_string));

// 5. Pass small types by value, large by const& (§3.5) — don't copy what you only read.
```

### 18.3 Compile-time computation with `constexpr` **[A]**
...
### 18.4 Profiling and benchmarking **[A]**
...
### 19.1 Dangling references, pointers, and iterators **[I/A]**
...
### 19.2 Object slicing **[I/A]**

Assigning a derived object to a base *value* slices off the derived part and loses virtual dispatch (§12.2). Handle polymorphic objects only through references, pointers, or `unique_ptr<Base>`.

### 19.3 Integer and conversion pitfalls **[I/A]**
...
### 19.5 `auto` and lambda pitfalls **[I/A]**
...
- **Capturing by reference in a lambda that outlives the scope** dangles (§4.3). For lambdas stored or run later (callbacks, threads), capture by value or move; never `[&]` into a stored lambda.

### 19.6 The best-practices checklist **[I/A]**
...
2. **The core of safe C++ (§5–7):** classes, RAII, the Rule of 0/3/5, smart pointers (`unique_ptr` first, then `shared_ptr`/`weak_ptr`), and move semantics. *Spend the most time here* — this is what separates modern C++ from C-with-classes.
3. **The STL (§8–9):** containers, then iterators, algorithms, and C++20 ranges. Learn to express logic as range pipelines rather than raw loops.
4. **Generics & errors (§10–11):** templates, concepts, and the trio of exceptions / `optional` / `expected`.
5. **Polymorphism, OS, concurrency (§12–14):** virtual functions vs `std::variant`, `<filesystem>` and file I/O, then threads/`jthread`/mutexes/`async`.
6. **Professional practice (§15–18):** CMake + vcpkg, sanitizers + clang-tidy + Core Guidelines, GoogleTest/Catch2 + CI, and profiling. This is what makes your C++ *production-grade*.

**Build-to-learn projects (in increasing difficulty):**

- **A RAII resource wrapper.** Write a class that owns a C resource (a `FILE*`, a socket, a `malloc`'d buffer) with a proper Rule-of-5 (move-only). Then *delete* it and replace it with `unique_ptr` + a custom deleter — feel how much boilerplate RAII saves. *Exercises:* §5, §6, §7.
- **A small `Vector<T>` container.** Implement a growable contiguous container: `push_back` with capacity doubling, `reserve`, iterators, copy/move (Rule of 5), strong exception safety. Run it under ASan/UBSan. *Exercises:* §5–§10, §16. This single project teaches more than any tutorial.
- **A thread pool.** A fixed set of `jthread`s pulling tasks (`std::function<void()>`) from a mutex+condition-variable-protected queue, with `std::future` results via `std::packaged_task`. Test it under TSan. *Exercises:* §14, §16.
- **A CLI tool.** Parse arguments, read/write files via `<filesystem>` and `<fstream>`, maybe shell out to a command, format output with `std::print`. Build it with CMake; add tests. *Exercises:* §13, §15, §17.
- **A JSON parser.** Parse JSON text into a `std::variant`-based value tree (or a recursive struct), returning `std::expected<Value, ParseError>`. Use `string_view` for zero-copy tokenizing, ranges for transforms, and write a thorough test suite. *Exercises:* §8–§12, §16, §17 — a capstone that uses nearly everything.
