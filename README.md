...
| [Next.js Full-Stack (Banking-Grade)](library/NEXTJS_FULLSTACK_APP_GUIDE.md) | **Next.js 16** + React 19 + **Prisma 6** + Supabase + **Better Auth v1** + Node 24 |
...
| [Node WebSockets (50k+)](library/NODE_WEBSOCKETS_GUIDE.md) | **ws v8** · **Socket.IO v4** · **uWebSockets.js** · NestJS 11 · Fastify v5 · Express 5 · Redis adapter |
| [NestJS GraphQL](library/NESTJS_GRAPHQL_GUIDE.md) | **@nestjs/graphql v13** + **Apollo v4/v5** (code-first) + Prisma 6 + Node 24 |
| **Databases & caching** | |
...
| [Go Coder WebSocket (realtime)](library/GO_CODER_WEBSOCKETS_GUIDE.md) | **coder/websocket v1.8.x** (ex-nhooyr, context-first) |
| [Go ent ORM](library/GO_ENT_ORM_GUIDE.md) | **ent v0.14+** |
| [Goose — DB migrations](library/GO_GOOSE_MIGRATIONS_GUIDE.md) | **pressly/goose v3.27+** |
| [sqlc + goose — type-safe SQL](library/GO_SQLC_GOOSE_GUIDE.md) | **sqlc v1.31+** + goose v3.27+ (pgx/v5) |
| [Go JWT + Argon2 (Banking-Grade Auth)](library/GO_JWT_ARGON2_GUIDE.md) | **golang-jwt/jwt v5** + `x/crypto/argon2`; **pquerna/otp** (TOTP), **go-webauthn** (passkeys), **x/oauth2 + go-oidc**, go-redis |
| [Go FS/OS/CLIs](library/GO_FILESYSTEM_OS_CLI_GUIDE.md) | stdlib + Cobra/Viper |
| [Go GraphQL](library/GO_GRAPHQL_GUIDE.md) | **gqlgen v0.17.x** (schema-first/codegen) + DataLoader + pgx |
| [Go gRPC & RPC](library/GO_GRPC_RPC_GUIDE.md) | **proto3**, `grpc` v1.6x+, `protobuf` v1.3x+ |
| [Go (Gin+Ent) + Next.js Full-Stack (RBAC CRUD + Realtime)](library/GO_GIN_NEXTJS_REALTIME_FULLSTACK_GUIDE.md) | **Gin v1.10+** · **Ent v0.14+** · **Gorilla WS v1.5** · golang-jwt v5 · Argon2id · **Next.js 16** · React 19 · TanStack Query v5 · Supabase Postgres |
| [Real-Time Chat Capstone (Go + Next.js)](library/GO_NEXTJS_REALTIME_CHAT_CAPSTONE_GUIDE.md) | **Gin · Ent v0.14 · goose v3.27 · pgx v5 · coder/websocket v1.8 · golang-jwt v5 · Argon2id · Air** + **Next.js 16** · React 19 · TanStack Query v5 · Redis · Nginx |
| **Auth & platforms** | |
...
| [C++](library/CPP_GUIDE.md) | **C++23** (C++26 flagged), modern C++ not C-with-classes: build/CMake, fundamentals & `auto`, **operators/control flow/console I/O** (`std::print`/`cin`, `if`/`switch`/range-`for`), functions & lambdas, **classes & RAII** (Rule of 0/3/5), **smart pointers & ownership**, **move semantics & value categories**, the **STL** (containers/iterators/**ranges**), **templates & concepts**, error handling (**`std::expected`**/exceptions/`optional`), polymorphism & `variant`, **`<filesystem>`/OS/processes**, concurrency (`jthread`/atomics/coroutines), **CMake/vcpkg/Conan**, **production C++** (sanitizers/Core Guidelines), GoogleTest/Catch2, performance | 2195 |
...
| [Next.js Full-Stack App — Production, Banking-Grade](library/NEXTJS_FULLSTACK_APP_GUIDE.md) | The **capstone** that composes the library into one standalone production app: architecture (RSC/Server Actions/Route Handlers as your backend), **Prisma + PostgreSQL** (singleton, migrations), **Supabase** (`@supabase/ssr`, **RLS in depth**, storage), data/mutations (Server Actions + TanStack Query + RHF/Zod), **auth with Better Auth** (Argon2, secure cookies/CSRF), **OTP**, **MFA/TOTP + recovery codes**, **OAuth/PKCE**, **RBAC + row-level ownership**, **realtime WebSockets** (separate WS service/Supabase Realtime + Redis), a big **banking-grade security** section (XSS/CSP, CSRF, IDOR, SSRF, rate limiting, headers, file/input validation, encryption, the server/client trust boundary), performance/caching, observability, testing (Vitest/Playwright), Docker/Vercel deploy | 2595 |
...
| Guide | What it covers | Lines |
|---|---|---|
| [React Native + Expo](library/REACT_NATIVE_EXPO_GUIDE.md) | How RN works (Metro/Hermes/JSI/**New Architecture**), RN vs native vs Flutter, **core components** & native mapping, styling/flexbox, **Expo Router**, device/SDK APIs, **FlatList performance**, Reanimated & gestures, platform-specific code, **EAS Build/Submit/Update (OTA)**, testing, shipping — **plus a banking-grade production half**: **secure/encrypted storage** (Keychain/Keystore, envelope encryption, encrypted SQLite/MMKV, **encrypted query-cache** with eviction), **biometric auth** (key-bound Face ID/Touch ID, step-up, enrollment-change invalidation), the **complete device-API & permissions reference** (contacts/calendar/clipboard/network/ATT… + iOS usage strings & Android permissions), **mobile security hardening** (TLS/SPKI **pinning**, jailbreak/root + **Play Integrity/App Attest**, screenshot/backgrounding privacy, deep-link & session hardening, OWASP MASVS), and **production App Store/Play Store release** (signing, **Privacy Manifest**/Data Safety, fintech review, staged rollout) | 4350 |
...
| [WebSockets in Node (Express/Fastify/NestJS) — 50k+ connections](library/NODE_WEBSOCKETS_GUIDE.md) | The WS protocol & the Node event loop (C10k→C10M), the landscape (**`ws`** vs **Socket.IO** vs **uWebSockets.js**), raw `ws` server/client, **authenticating the socket** (Origin/CSWSH), lifecycle (heartbeats/backpressure/graceful drain), message protocol design, **Express 5 / Fastify v5 / NestJS 11 gateways / Socket.IO** each in depth, and the centerpiece — **scaling to 50,000+ connections**: FD/`ulimit`/sysctl tuning, **horizontal scaling with a Redis backplane**, sticky sessions, uWebSockets.js, **Nginx WS load-balancing**, backpressure at scale, **load-testing to 50k**, and banking-grade hardening | 1795 |
| [GraphQL with NestJS](library/NESTJS_GRAPHQL_GUIDE.md) | **Code-first** `@nestjs/graphql v13` + Apollo: types via decorators, resolvers & **field resolvers**, class-validator + error filters, **N+1 & DataLoader**, **Prisma** integration, **cursor/Relay pagination**, **subscriptions** (graphql-ws + **Redis** backplane), **auth Guards via `GqlExecutionContext` + RBAC**, a full **banking-grade GraphQL security** section (depth/complexity limits, throttling, introspection off, APQ/allow-listing, batching-abuse, error hygiene), caching/perf, **federation**, testing, deploy | 1840 |
...
| [Go Gin — Production-Grade RESTful Backend](library/GO_GIN_REST_API_FILE_UPLOAD_GUIDE.md) | **Gin in depth** (engine/router/`*gin.Context`/middleware chain), RESTful design, binding/validation, **both folder structures (layer-based & feature-based)**, architecture & **design patterns** (handler→service→repository, DI, factory, options, adapter), middleware, **auth in depth** (Argon2id password hashing, JWT access + **refresh-cookie rotation with reuse detection & revocation**, RBAC), DB layer (**pgx and Ent ORM**, tx, migrations), **realtime updates with WebSockets** (auth, hub, push-from-service, Redis-backplane scaling), config/secrets, observability, error handling, **file uploads**, performance/scaling, **security hardening**, testing, Docker deploy, a complete worked Users+Orders API, live reload with Air | 3978 |
...
| [Go Coder WebSocket — Thread-Safe Realtime](library/GO_CODER_WEBSOCKETS_GUIDE.md) | **`github.com/coder/websocket`** (the modern, context-first, zero-dependency library; ex-`nhooyr.io/websocket`) from zero→production realtime: the WS protocol & 101 handshake, **why coder vs gorilla**, the API in depth (`Accept`/`Dial`/`Read`/`Write`/`wsjson`/`CloseStatus`/`SetReadLimit`), the **concurrency model** (the one-reader rule + **internally-serialized concurrent writes** — the "thread-safe" headline), the production **Hub** (read/write pumps, bounded send channel, **backpressure & slow-consumer eviction**, heartbeats, graceful drain), **Gin** integration, **authenticating the upgrade** (JWT via **subprotocol**/ticket since browsers can't set headers; Argon2 login), **Ent + pgx** persistence, **Air**, a full **banking-grade security** section (CSWSH/Origin, per-message authz, rate limits, DoS/max-conns, TLS/wss), **Redis-backplane scaling**, tests | 1924 |
...
| [Goose — Database Migrations](library/GO_GOOSE_MIGRATIONS_GUIDE.md) | **`pressly/goose v3.27+`** zero→production: what migrations are & **why never auto-migrate in prod**, SQL migrations (Up/Down, **`StatementBegin/End`** for functions/triggers, **`NO TRANSACTION`** for `CREATE INDEX CONCURRENTLY`), the full command reference, **Go migrations** (data backfills), **embedding with `embed.FS`**, the modern **`NewProvider`** API (`WithSlog`/`WithTableName`), **pgx integration** (`stdlib.OpenDBFromPool`), **coexisting with Ent** (goose owns the schema, Ent migration off), and a banking-grade production workflow (deploy-step vs boot, **advisory-lock runner** vs split-brain, **expand/contract zero-downtime**, roll-forward), inside a Gin + Ent + pgx + JWT/Argon2 + Air app | 1707 |
| [sqlc + goose — Type-Safe SQL & Migrations](library/GO_SQLC_GOOSE_GUIDE.md) | **`sqlc v1.31+` + `goose v3.27+`**: write real SQL → get **compile-time-checked, reflection-free Go**, with **goose migrations as sqlc's schema source** (sqlc parses the `-- +goose` annotations natively so codegen always matches the live DB — the killer combo). `sqlc.yaml` (**pgx/v5**) in depth, all query annotations (`:one`/`:many`/`:exec`/`:copyfrom`/`:batch*`, `sqlc.arg`/`narg`/`embed`), the generated `Queries`/`WithTx`, **transactions** (atomic money transfer), the CI loop (`sqlc vet`/`sqlc diff`), the **one-pgxpool / two-query-layers** pattern (sqlc for hot paths + Ent for the graph), **banking-grade** (`numeric` money, injection-proof by construction, row-level authz), a worked **accounts + ledger** service | 2515 |
| [Go JWT + Argon2 — Banking-Grade Auth & Security](library/GO_JWT_ARGON2_GUIDE.md) | Password-hashing threat model, **Argon2id** (PHC format, parameter tuning, constant-time verify), JWT structure & golang-jwt v5 with **rigorous algorithm validation**, **access/refresh rotation & revocation**, auth middleware (net/http & Gin), **RBAC**, HMAC vs RSA/ECDSA, key management, token-storage security — **plus a banking-grade security half**: **OTP via email & WhatsApp** (Cloud API), **MFA** (TOTP + recovery codes), **OAuth2/OIDC** (PKCE), **Passkeys/WebAuthn**, **encryption in transit & at rest** (TLS 1.3/mTLS, AES-GCM, envelope/KMS), **rate limiting** (Redis), **file/image upload validation** (anti-hidden-payload), and **payload/multipart validation** — all with **DB schemas** and implemented to banking-grade standards | 5221 |
...
| [GraphQL with Go (gqlgen)](library/GO_GRAPHQL_GUIDE.md) | GraphQL vs REST/gRPC, the **type system & SDL**, **gqlgen** schema-first codegen & resolvers, error handling, **the N+1 problem & DataLoader** (per-request batching), mutations/validation, **cursor/Relay pagination**, **subscriptions** (graphql-ws + **Redis** scaling), **auth & field-level RBAC** (directives/resolver guards), a thorough **banking-grade GraphQL security** section (depth limiting, **complexity/cost analysis**, rate limiting, introspection off, **persisted/allow-listed queries**, batching/alias DoS, error hygiene), performance/caching, **federation**, testing (Testcontainers), Docker/observability deploy | 2868 |
...
| [Go (Gin+Ent) + Next.js Full-Stack — RBAC CRUD & Realtime](library/GO_GIN_NEXTJS_REALTIME_FULLSTACK_GUIDE.md) | The **integration capstone** that composes the Go + Next.js stack into one decoupled system around a **Courses** resource: the **decoupled architecture** (Go API + Next.js client, REST + WebSocket), monorepo layout, **Gin** engine/config/graceful shutdown, **Supabase Postgres** (pooler vs direct, pgx+Ent), **Ent** schema (users/courses/role enum, author edge), **Argon2id** + signup/login, **JWT access+refresh** (rotation, HttpOnly cookie, rigorous alg checking) + auth middleware, **RBAC** (permissions map, authorize middleware, **row-level ownership**/IDOR defense, admin user-management), the **CRUD REST API** (DTOs, handler→service split, status codes), **CORS** & the TS↔Go contract, **Gorilla WebSocket** realtime (the **Hub**, **authenticating the WS upgrade**, CheckOrigin/CSWSH, heartbeats, backpressure) and **broadcasting CRUD events**, then the **Next.js dashboard**: in-memory access token + auto-refresh interceptors, route guards, **TanStack Query** data layer, the **Course CRUD UI** (forms, **optimistic updates** + rollback), **consuming WebSocket realtime in React** (cache-as-source-of-truth, idempotency, reconnect+refetch), **frontend RBAC** (conditional UI/route guards mirroring the server), a full **end-to-end trace**, **full-stack security hardening**, and **deployment** (Docker, one-origin reverse proxy, Redis backplane for scale) | 3927 |
| [Real-Time Chat Capstone — Go + Next.js (all tools together)](library/GO_NEXTJS_REALTIME_CHAT_CAPSTONE_GUIDE.md) | The **grand capstone** that composes the realtime stack into one production **chat application**: **Gin · Ent v0.14 · goose · pgx v5 · coder/websocket · JWT/Argon2 · Air** backend + **Next.js 16 / React 19 / TanStack Query v5** frontend. The **data-layer design** (one `pgxpool` → **goose** owns the schema, **Ent** is the single type-safe query layer for *every* read and write — graph CRUD *and* the message hot path via typed predicates, compound-keyset pagination, aggregation, and a raw-SQL `.Modify` escape hatch — **pgx** owns the sockets), goose schema (users/rooms/room_members/messages/read_receipts, DMs-as-rooms), **Argon2id + JWT rotation** with reuse detection, **room-role RBAC + IDOR defense** (Ent `HasMembersWith` EXISTS guard), the Gin REST API, the **coder/websocket Hub** (auth-on-upgrade via subprotocol, CSWSH/`OriginPatterns`, read/write pumps, backpressure, drain), the **persist-then-broadcast** send flow (Ent tx + `client_msg_id` idempotency via constraint-error read-back), **presence & typing** (Redis TTL), **unread counts & read receipts**, **Redis pub/sub backplane + Nginx one-origin** scaling, the **Next.js** client (in-memory token + silent refresh, infinite keyset history, **WS-events-into-the-TanStack-cache**, optimistic send + rollback), the **XSS-safe** chat UI, frontend RBAC mirror, a large **banking-grade security** section, observability/testing (Dial-based WS tests, Ent tx-rollback), **Docker/Compose** deploy, and a both-directions **end-to-end trace** | 4171 |
...
[JavaScript](library/JAVASCRIPT_GUIDE.md) → [Node.js](library/NODEJS_GUIDE.md) → [Next.js 16](library/NEXTJS_16_GUIDE.md) → [Fastify](library/FASTIFY_GUIDE.md) / [NestJS](library/NESTJS_GUIDE.md) → [PostgreSQL](library/POSTGRESQL_GUIDE.md) → [Prisma](library/PRISMA_ORM_GUIDE.md) → [Redis](library/REDIS_GUIDE.md) → [Docker](library/DOCKER_GUIDE.md). **Capstone:** [Next.js Full-Stack App (banking-grade)](library/NEXTJS_FULLSTACK_APP_GUIDE.md) — composes Prisma/Supabase/Better Auth/OTP/MFA/OAuth/RBAC/WebSockets into one production app. For APIs: [GraphQL with NestJS](library/NESTJS_GRAPHQL_GUIDE.md); for realtime at scale: [WebSockets in Node (50k+)](library/NODE_WEBSOCKETS_GUIDE.md).

**④ Backend with Go**
[Go — Language & Patterns](library/GO_LANG_AND_PATTERNS_GUIDE.md) → [Go net/http](library/GO_NET_HTTP_REST_API_GUIDE.md) → [Gin + uploads](library/GO_GIN_REST_API_FILE_UPLOAD_GUIDE.md) → [JWT + Argon2](library/GO_JWT_ARGON2_GUIDE.md) → [PostgreSQL](library/POSTGRESQL_GUIDE.md) → [ent ORM](library/GO_ENT_ORM_GUIDE.md) → [Goose migrations](library/GO_GOOSE_MIGRATIONS_GUIDE.md) → [sqlc + goose](library/GO_SQLC_GOOSE_GUIDE.md) → [Redis](library/REDIS_GUIDE.md) → realtime with [Gorilla WebSockets](library/GO_GORILLA_WEBSOCKETS_GUIDE.md) **or** the modern [Coder WebSocket](library/GO_CODER_WEBSOCKETS_GUIDE.md) (context-first, thread-safe writes) → [GraphQL (gqlgen)](library/GO_GRAPHQL_GUIDE.md) → [gRPC & RPC](library/GO_GRPC_RPC_GUIDE.md) → [File System / OS / CLIs](library/GO_FILESYSTEM_OS_CLI_GUIDE.md) → [Docker](library/DOCKER_GUIDE.md). **Capstones (Go + Next.js together):** [Go (Gin+Ent) + Next.js Full-Stack — RBAC CRUD & Realtime](library/GO_GIN_NEXTJS_REALTIME_FULLSTACK_GUIDE.md) composes Gin/Ent/Argon2/JWT/Gorilla-WS on Supabase Postgres with a Next.js admin dashboard (REST + WebSocket realtime, RBAC, optimistic CRUD); and the grand [Real-Time Chat Capstone](library/GO_NEXTJS_REALTIME_CHAT_CAPSTONE_GUIDE.md) composes the stack (Gin · **Ent** as the single data layer · **goose** · pgx · **coder/websocket** · JWT/Argon2 · Air + Next.js) into a scalable, banking-grade realtime chat with a Redis backplane. Pair with the [React-ecosystem track ②](#-suggested-learning-tracks) for the frontend half.
...
- **Realtime & data-layer guides (2026 batch).** The four newest Go guides — **Coder WebSocket**, **Goose migrations**, **sqlc + goose**, and the **Real-Time Chat Capstone** — were written for 2026 accuracy against verified library APIs (coder/websocket v1.8.x, goose v3.27+, sqlc v1.31+, pgx v5). Their **131 ` ```go ` blocks were parsed with the Go toolchain** (`go/parser`, Go 1.26). One real API slip was caught and fixed — a gorilla-style `websocket.Subprotocols()` helper that **does not exist** in coder/websocket, replaced with manual `Sec-WebSocket-Protocol` header parsing (in two guides). The remaining handful of parser flags are intentional teaching snippets that interleave declarations with usage statements (or show a lone struct-field fragment) — valid in context, not compilable as a stand-alone unit. **TypeScript/TSX, SQL, and exact third-party symbol names were not compiler-checked** — confirm against official docs before production use.
