...
| [Real-Time Chat Capstone (Go + Next.js)](library/GO_NEXTJS_REALTIME_CHAT_CAPSTONE_GUIDE.md) | **Gin · Ent · sqlc v1.31 · goose v3.27 · pgx v5 · coder/websocket v1.8 · golang-jwt v5 · Argon2id · Air** + **Next.js 16** · React 19 · TanStack Query v5 · Redis · Nginx |
| **Auth & platforms** | |
...
| [Real-Time Chat Capstone — Go + Next.js (all tools together)](library/GO_NEXTJS_REALTIME_CHAT_CAPSTONE_GUIDE.md) | The **grand capstone** that composes the entire realtime stack into one production **chat application**: **Gin · Ent · sqlc · goose · pgx · coder/websocket · JWT/Argon2 · Air** backend + **Next.js 16 / React 19 / TanStack Query v5** frontend. The **data-layer division of labor** (one `pgxpool` → **goose** owns the schema, **sqlc** owns the hot paths, **Ent** walks the graph, **pgx** owns the sockets), goose schema (users/rooms/room_members/messages/read_receipts, DMs-as-rooms), **Argon2id + JWT rotation** with reuse detection, **room-role RBAC + IDOR defense**, the Gin REST API, the **coder/websocket Hub** (auth-on-upgrade via subprotocol, CSWSH/`OriginPatterns`, read/write pumps, backpressure, drain), the **persist-then-broadcast** send flow (`client_msg_id` idempotency), **presence & typing** (Redis TTL), **unread counts & read receipts**, **Redis pub/sub backplane + Nginx one-origin** scaling, the **Next.js** client (in-memory token + silent refresh, infinite keyset history, **WS-events-into-the-TanStack-cache**, optimistic send + rollback), the **XSS-safe** chat UI, frontend RBAC mirror, a large **banking-grade security** section, observability/testing (Dial-based WS tests, tx-rollback), **Docker/Compose** deploy, and a both-directions **end-to-end trace** | 4069 |
